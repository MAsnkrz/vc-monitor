...
How it works:
  ...
  2. Compares against snapshot.json from the previous run
  3. Fires Discord embeds for any changes found
  4. Saves a fresh snapshot so the next run can compare
...
def variant_fields(v, barcode, qty, old_qty=None):
    """Standard fields for a variant — price, SKU, barcode, qty, pack size."""
    qty_value = "—"
    if qty is not None:
...
def notify_new_product(product, new_state):
    ...
    for vid, v in list(variants.items())[:4]:
        ...
        fields.append(barcode_field(barcodes.get(vid, "")))
